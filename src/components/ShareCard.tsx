...
export function ShareCard({ fortune, dateStr, onClose }: ShareCardProps) {
  ...
  const generateCard = async () => {
    ...
    // ── FOOTER: 爻光 + Cyber Oracle + 醒目网址 + slogan ──

    // 爻光 大标题
    ctx.save();
    const brandGrad = ctx.createLinearGradient(58, 0, 360, 0);
    ...
    ctx.font = '20px "Share Tech Mono", monospace';
    ctx.textAlign = 'left'; ctx.textBaseline = 'top';
    ctx.fillText('Cyber Oracle', 58, curY + 62);
    ctx.restore();

    // 网址 — 醒目金色圆角背景 pill 样式
    const urlText = 'cyberoracle.lovable.app';
    ctx.save();
    ctx.font = '700 26px "Share Tech Mono", monospace';
    const urlW = ctx.measureText(urlText).width + 48;
    const urlH = 52; const urlX = 58; const urlY = curY + 96;
    // 背景 pill
    ctx.fillStyle = 'rgba(200,169,110,0.12)';
    ctx.strokeStyle = 'rgba(200,169,110,0.5)';
    ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.roundRect(urlX, urlY, urlW, urlH, 26); ctx.fill(); ctx.stroke();
    // 文字
    ctx.fillStyle = '#E8C88A';
    ctx.shadowColor = 'rgba(200,169,110,0.5)'; ctx.shadowBlur = 10;
    ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
    ctx.fillText(urlText, urlX + 24, urlY + urlH / 2);
    ctx.restore();
    ...
    ctx.fillStyle = '#5C5480';
    ctx.textAlign = 'left'; ctx.textBaseline = 'top';
    ctx.fillText('一爻一光，日日新启', 58, curY + 164);
    ctx.restore();
  ...
  const handleCopyUrl = async () => {
    try {
      await navigator.clipboard.writeText('https://cyberoracle.lovable.app');
